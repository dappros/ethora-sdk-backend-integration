...
function createScopedJwtToken(
  ...
  const secrets = getSecrets();
  return jwt.sign(payload, deriveScopedSecret(secrets.chatAppSecret, purpose), {
    algorithm: "HS256",
  });
}
...
export function createServerToken(): string {
  ...
  return createScopedJwtToken(payload, "server");
}
...
export function createClientToken(userId: string): string {
  ...
  const token = createScopedJwtToken(payload, "client");
